...
// Upgrade NOTE: Added clamps to the output to avoid unwanted transparency 	
// Upgrade NOTE: replaced 'samplerRECT' with 'sampler2D'

//from http://forum.unity3d.com/threads/68402-Making-a-2D-game-for-iPhone-iPad-and-need-better-performance

Shader "Futile/ColoredSprite2Depth" //Unlit Transparent Vertex Colored Additive 
{
Properties 
	{
		...
		SubShader   
		{
			...
			Pass
			{
				...
				half4 frag (v2f i) : SV_Target
				{
					...
					half terrainDpth = ((uint)round(tex2D(_LevelTex, textCoord).x * 255) % 30)/30.0;
					...
					return float4(clamp(returnCol.xyz,0.003921568627451,1),  tex2D(_MainTex, i.uv).w);
				}
				ENDCG
			}
		} 
