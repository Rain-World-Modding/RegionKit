...
public class DenseFogGradient : CosmeticSprite
{
	...
	public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
	{
		...
		sLeaser.sprites = new FSprite[] { new("pixel") { scaleX = 1500f, scaleY = 900f, anchorX = 0f, anchorY = 0f } };
	...
	public override void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
	{
		...
		if (rm.roomSettings.GetEffectAmount(_Enums.DenseFogSoundVolume) <= 0f)
			_danger = 2;
		if (_danger == 0 && s0.alpha >= .8f)
			++_danger;
        FloatRect roomRect = rm.RoomRect;
        float volumeEffectAmount = rm.roomSettings.GetEffectAmount(_Enums.DenseFogSoundVolume),
			fogSoundVolume = (rm.roomSettings.GetEffect(_Enums.DenseFogSoundVolume) is null) ? s0.alpha : volumeEffectAmount * ((float)rm.world.rainCycle.timer / rm.world.rainCycle.cycleLength),
			fogSoundVolume2 = Mathf.Max(0f, fogSoundVolume - .5f);
        if (_danger == 1 && s0.alpha < .99f)
		{
