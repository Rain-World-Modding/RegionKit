...
public static class _Module
{
	...
	internal static void __DevUIElementReceivedRMB(DevInterface.DevUINode node)
	{
		...
		{
			LogError($"Iggy error when checking for tooltips for {node} {node?.IDstring}");
			LogError(ex);
			__messageSystem.PlayMessageNow(new(TimeSpan.FromSeconds(20), $"Ouch! Something broke: {ex.Message}! I logged the full exception text.", false));
		}
	}
