...
internal static class RainSong
{
    ...
    static void RainUpdatePatch(On.RainCycle.orig_Update orig, RainCycle self)
    {
        ...
        if (self.RainApproaching < 0.5f && player != null && game.cameras[0].room.roomSettings.DangerType != RoomRain.DangerType.None && player.song == null && !game.cameras[0].room.abstractRoom.name.Contains("GATE") && !game.Players[0].realizedCreature.dead && rainSongDict.TryGetValue(self.world.region.name, out string songName))
        {
